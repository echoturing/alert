...
// The DatasourceQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type DatasourceQueryRuleFunc func(context.Context, *ent.DatasourceQuery) error

// EvalQuery return f(ctx, q).
func (f DatasourceQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.DatasourceQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.DatasourceQuery", q)
}

// The DatasourceMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type DatasourceMutationRuleFunc func(context.Context, *ent.DatasourceMutation) error

// EvalMutation calls f(ctx, m).
func (f DatasourceMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.DatasourceMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.DatasourceMutation", m)
}
